...
| **The deal name fills itself in as "Customer - City"** on New deal and Convert to deal, and stays editable. Once somebody types their own ("Flipkart - GGN") it stops following; clearing the box hands it back. With several cities it is the customer alone, and the server adds each city. Deal name sits BELOW the city on New deal, where it can be seen filling in. | Vivek: Customer and Deal name are different things — the customer groups every Flipkart deal, the name tells three of them apart — but nobody should have to type the second. Still required, so a deal can never be saved without one. |
| **Duplicate this deal** (deal page) opens the New deal sheet filled with the deal's customer, city, vehicle, fleet, price, driver, charging, operating days, closing month (if still offered), notes and owner — as a NEW deal at First Contact, name suggested afresh, no cost sheet. It reaches the shell's sheet through an event (`lib/new-deal.ts`), like `showToast()`. | Vivek asked for it. Distinct from Deploy more vehicles (expansion of a WON deal, linked, counted as won the day raised). Costs are not copied because a copy of a costed deal would look costed without anyone having looked. |
| **Settings carries "How the CRM works"** (`components/faq.tsx`): role-filtered questions, plain `<details>`. | Vivek wanted users to understand the app's behaviour without asking. **Every answer describes current behaviour — change a rule, change its answer.** |
| **Forecast is titled "Sales Closure Forecast"** with "expected sales closure month for all open deals" under it; **Deployments says** all Contracting and Closed Won deals with an expected deployment date show there. Both notes show on phones too. | Vivek's wording; the Deployments note answers ops' first question — why is an unsigned deal here. |
...
| **An assignment pushes a notification to the assignee's devices**, never email: title "New lead: ZYRKON · Hyderabad · 2 × 3W", body the caller and number, and a **Call** button when there is a number. Web Push with VAPID keys; one `push_subscriptions` row per browser; a 404/410 deletes the row. Best effort — never fails the assignment. | Vivek: in-app plus push, no email. Assigning to yourself notifies you too — you assign at a laptop and ring from a phone (an early rule skipped it, and the first real test looked like a failure). A notification cannot dial by itself (a service worker cannot open `tel:`), so Call opens `/leads?lead=<id>&call=1`: the page scrolls to the card, rings it green, and hands the number to the dialer; where a browser refuses to dial without a tap, the card's Call button is under the thumb. A plain tap opens `/leads?lead=<id>`. iOS shows no notification buttons at all. |
